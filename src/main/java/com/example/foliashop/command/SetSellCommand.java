...
public class SetSellCommand implements CommandExecutor {
    private final FoliaShopPlugin plugin;
    private final ShopPointDao dao;

    public SetSellCommand(FoliaShopPlugin plugin, ShopPointDao dao) { this.plugin = plugin; this.dao = dao; }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        FoliaSchedulerUtil.runAtLocation(plugin, place, () -> {
            ...
            ShopPoint sp = new ShopPoint(0, place.getWorld().getName(), place.getBlockX(), place.getBlockY(), place.getBlockZ(),
                    "SELL", p.getLocation().getYaw(), p.getLocation().getPitch(), p.getUniqueId().toString());
            try {
                dao.insert(sp);
            } catch (SQLException e) {
                plugin.getLogger().severe("DB insert error: " + e.getMessage());
            }
            plugin.msg().send(p, "sell-placed", Map.of(
